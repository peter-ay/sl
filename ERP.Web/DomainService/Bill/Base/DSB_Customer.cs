...
namespace ERP.Web.DomainService.Bill
{
    ...
    [EnableClientAccess()]
    public class DSB_Customer : DomainService
    {
        private BLLBase bll = new BB_Customer();

        [Invoke]
        public string Add(string dbCode, int lgIndex, MB_Customer t)
        {
            return bll.Add(dbCode, lgIndex, t);
        }

        [Invoke]
        public void Update(string dbCode, int lgIndex, MB_Customer t)
        {
        ...
        public void AssignRightBrowse(string dbCode, int lgIndex, string gpCode, List<string> codes, bool f_ADD = false)
        {
            ...
            try
            {
                ...
                {
                    strSql.Clear();
                    strSql.Append("update B_Customer  ");
