...
{
    public class ImportB_Material_Lens1 : ImportBase
    {
        protected override void PrepareImport(SqlCommand cmd, DataSet ds)
        {
            ...
            //
            strSql.Clear();
            strSql.Append("select top 0 * into #B_Material_Lens1 from B_Material_Lens where LensLevel=1;");
            ...
            {
                strSql.Clear();
                strSql.Append("Delete #B_Material_Lens1 where LensCode=@LensCode;");
                //
                strSql.Append("insert into #B_Material_Lens1(");
            ...
            //
            strSql.Clear();
            strSql.Append("delete B_Material_Lens where LensLevel=1;");
            strSql.Append("insert into B_Material_Lens select * from #B_Material_Lens1;");
            cmd.CommandText = strSql.ToString();
            cmd.ExecuteNonQuery();
        }
    }
