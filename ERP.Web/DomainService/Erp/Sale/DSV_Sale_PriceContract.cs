...
namespace ERP.Web.DomainService.Erp
{
    ...
    {
        public IQueryable<V_Sale_PriceContract> GetV_Sale_PriceContractBill(string dbCode, string iD)
        {
            this.ObjectContext.ChangeDataBase(dbCode);
            if (iD == "") return this.ObjectContext.V_Sale_PriceContract;
            return this.ObjectContext.V_Sale_PriceContract.Where(item => item.ID == iD);
        }

        public IQueryable<V_Sale_PriceContract> GetV_Sale_PriceContractList(string dbCode, string sWhere)
        {
            this.ObjectContext.ChangeDataBase(dbCode);

            IQueryable<V_Sale_PriceContract> _Rs = this.ObjectContext.V_Sale_PriceContract;
            ...
            if (!string.IsNullOrEmpty(_Str))
            {
                _Str.Split('%').ToList().ForEach(it => { _Rs = _Rs.Where(item => item.CusGroup.Contains(it)); });
            }
            ...
            if (!string.IsNullOrEmpty(_Str))
            {
                _Str.Split('%').ToList().ForEach(it =>
                {
                    ...
                    _Rs = _Rs.Where(item => _SCode.Contains(item.CusGroup));
                });
            }
            ...
            if (!string.IsNullOrEmpty(_Str))
            {
                _Str.Split('%').ToList().ForEach(it =>
                {
                    var _SCode = (from c in this.ObjectContext.V_Sale_PriceContract_Lens
