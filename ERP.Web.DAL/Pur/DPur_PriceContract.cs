...
{
    public partial class DPur_PriceContract : DALBase
    {
        public DPur_PriceContract()
        { }

        public override bool Exists(string dbCode, int lgIndex, string vCode)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from Pur_PriceContract with (nolock)");
        ...
        {
            MPur_PriceContract model = t as MPur_PriceContract;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Pur_PriceContract(");
            strSql.Append("ID,BCode,OBCode,BDate,BType,PriCode,StCode,BegDate,EndDate,SpGroup,Remark,Maker,MName,MDate,Checker,ChName,ChDate,F_Del,Deler,DelName,DelDate,GpNameNew)");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@SpGroup", SqlDbType.VarChar,10),
            ...
            parameters[9].Value = model.SpGroup;
        ...
        private void PrepareSaveVerify(int lgIndex, SqlCommand cmd, MPur_PriceContract model)
        {
            StringBuilder strSql = new StringBuilder();
            SqlParameter[] parameters = null;
            strSql.Append("SP_Pur_PriceContract_SaveVerify");
        ...
        {
            var model = t as MPur_PriceContract;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Pur_PriceContract set ");
            strSql.Append("OBCode=@OBCode,");
            strSql.Append("PriCode=@PriCode,");
            strSql.Append("BegDate=@BegDate,");
            strSql.Append("EndDate=@EndDate,");
            strSql.Append("SpGroup=@SpGroup,");
            ...
            parameters[4].Value = model.SpGroup;
