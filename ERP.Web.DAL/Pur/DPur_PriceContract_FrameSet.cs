...
{
    public partial class DPur_PriceContract_FrameSet : DALBase
    {
        public DPur_PriceContract_FrameSet()
        { }

        public override bool Exists(string dbCode, int lgIndex, string vCode)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from Pur_PriceContract_FrameSet with (nolock)");
        ...
        {
            var model = t as MPur_PriceContract_FrameSet;
            StringBuilder strSql = new StringBuilder();
            //////////////////////////////////////////
            strSql.Append("delete Pur_PriceContract_FrameSet ");
            strSql.Append("where BID=@BID and FrameCode=@FrameCode and FQty=@FQty ");
            strSql.Append("and LensCode=@LensCode and LQty=@LQty ;");
            /////////////////////////////////////////
            strSql.Append("insert into Pur_PriceContract_FrameSet(");
            strSql.Append("ID,BID,FrameCode,FQty,LensCode,LQty,Price,Price_ProCost,InvTitle)");
            strSql.Append(" values (");
            strSql.Append("(select HKOERP.dbo.SF_GetID()),@BID,@FrameCode,@FQty,@LensCode,@LQty,@Price,@Price_ProCost,@InvTitle)");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.VarChar,25),
            ...
            parameters[8].Value = model.InvTitle;
