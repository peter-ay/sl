...
{
    public partial class DPur_PriceContract_Lens_ProCost : DALBase
    {
        public DPur_PriceContract_Lens_ProCost()
        { }

        public override bool Exists(string dbCode, int lgIndex, string vCode)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from Pur_PriceContract_Lens_ProCost with (nolock)");
        ...
        {
            var model = t as MPur_PriceContract_Lens_ProCost;
            StringBuilder strSql = new StringBuilder();
            //////////////////////////////////////////
            strSql.Append("delete Pur_PriceContract_Lens_ProCost ");
            ...
            /////////////////////////////////////////
            strSql.Append("insert into Pur_PriceContract_Lens_ProCost(");
            strSql.Append("ID,BID,LensCode,F_Set,InvTitle,JY,UV,JS,RS,CS,SY,CB,ChB,KK,ZK,PiH,PG,JJ,OP,P1,P2)");
            strSql.Append(" values (");
            strSql.Append("(select HKOERP.dbo.SF_GetID()),@BID,@LensCode,@F_Set,@InvTitle,@JY,@UV,@JS,@RS,@CS,@SY,@CB,@ChB,@KK,@ZK,@PiH,@PG,@JJ,@OP,@P1,@P2)");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.VarChar,25),
