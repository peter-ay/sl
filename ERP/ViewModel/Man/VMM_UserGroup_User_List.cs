...
{
    public class VMM_UserGroup_User_List : VMList
    {
        ...
        public VMM_UserGroup_User_List()
        ...
        {
            var ds = ComDDSFactory.Get(ComDSFactory.Man, UDSMethods.V_S_UserByGroupCodeList, ReSetSelectCodes);
            ds.QueryParameters.Add(new Parameter() { ParameterName = "gpCode", Value = this._gpCode });
            this.IsBusy = true;
            ds.Load();
        }
        ...
        private void UpdateCodes(bool flag, bool isShowBusy = false)
        {
            ...
            DS_Bill.Value.Update(this._gpID, uCodeList, flag,
                geted =>
                {
                    if (isShowBusy)
                        this.IsBusy = false;
