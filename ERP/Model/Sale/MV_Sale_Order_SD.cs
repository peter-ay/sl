...
namespace ERP.Web.Entity
{
    partial class V_Sale_Order_SD
    {
        ...
        partial void OnCusCodeChanged()
        {
            ...
            {
                this.DpCodeJG = _Rs.DpCodeCX ?? "";
            }
            _IsNoticeForRepeat = _Rs.F_NoticeRepeatOBill.Value;
        ...
        partial void OnLensCodeRChanged()
        {
            ...
            Messenger.Default.Send(_Rs.LensType == "RX", USysMessages.SDProcessUpdate);
        }
        ...
        partial void OnLensCodeRRChanged()
        {
            if (EditState != 1) return;
            this.LensNameRR = "";
            var _Rs = ComHelpLensCode.UHV_B_Material_LensSmart.Where(it => it.LensCode.MyStr() == this.LensCodeRR.MyStr()).FirstOrDefault();
            if (_Rs == null) return;
        ...
        partial void OnLensCodeRLChanged()
        {
            if (EditState != 1) return;
            this.LensNameRL = "";
            var _Rs = ComHelpLensCode.UHV_B_Material_LensSmart.Where(it => it.LensCode.MyStr() == this.LensCodeRL.MyStr()).FirstOrDefault();
            if (_Rs == null) return;
